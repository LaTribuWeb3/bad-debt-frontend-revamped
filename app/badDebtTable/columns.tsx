...
export const columns: ColumnDef<ProtocolResult>[] = [
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex items-center mx-auto max-w-[200px]">
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex flex-wrap items-center mx-auto max-w-[200px]">
          {chains.map((_, i) => (
            <div key={i} className="rounded-full overflow-hidden m-1">
              <Image src={`/images/chains/${_.toLowerCase()}.webp`} width={28} height={28} alt={'blockchain logo'} />
            </div>
          ))}
        </div>
      );
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <CustomTooltip content={formatCurrency(amount)}>
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="font-medium">
          {subResults ? (
            <Link href={`/subresults?platform=${platform}`}>
              Full dashboard for {Object.keys(subResults).length} markets
            </Link>
          ) : (
