...
export default async function GetGithubData(dateStr: string): Promise<ProtocolResult[]> {
    ...
    const response = await fetch('https://api.github.com/repos/Risk-DAO/simulation-results/git/trees/main?recursive=1', { cache: 'no-store' });
    if (!response.ok) {
    ...
    for (const protocolName of allProtocols) {
        ...
        if (protocolFileContents.length == 1) {
            ...
            protocolResults.push({
                ...
                badDebtRatio: normalize(protocolFileContents[0].total, decimals) / normalize(protocolFileContents[0].tvl, decimals)
            });
        } else {
            const decimals = Number(protocolFileContents[0].decimals);
